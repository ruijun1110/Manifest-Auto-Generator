...
/// 用于生成导出文件的模板类。
/// </summary>
public class YUEJIEMIA : ManifestTemplate
{
    // 模板文件的路径。
    private const string _templateFilePath = "WanbXLSTemplates/美国/YUEJIE MIA Manifest-94CTNS.xlsx";
    // 模版表头对应关系与默认值的json文件路径。（由于该模板有多个表单需要填写，因此需要分别指定。）
    private static readonly string[] _headersMatchingJson = new string[] { "TemplatesBinding/USTemplates/HeaderMatchings/YUEJIEMIAW1.json", "TemplatesBinding/USTemplates/HeaderMatchings/YUEJIEMIAW2.json" };

    public YUEJIEMIA() : base()
    {
        TemplateName = "YUEJIE MIA Manifest-94CTNS";
    }
    ...
    public override XLWorkbook GenerateFile(List<int> rowsToExport, List<Dictionary<string, object>> rows)
    {
        ...
        for (int i = 0; i < 2; i++)
        {
            ...
            {
                var rowToExport = rows[row - RowNumberDisplacement];
                foreach (var header in rowToExport.Keys)
                {
