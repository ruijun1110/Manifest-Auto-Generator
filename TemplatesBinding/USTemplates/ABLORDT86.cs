...
/// 用于生成导出文件的模板类。
/// </summary>
public class ABLORDT86 : ManifestTemplate
{
    // 模板文件的路径。
    private const string _templateFilePath = "WanbXLSTemplates/美国/ABL ORD T86 MANIFEST-65CTNS.xlsx";

    public ABLORDT86() : base()
    {
        BeginningRowNumber = 2;
        OverwriteWorksheetIndex = 1;
        TemplateHeadersExcelRange = "A2:AA2";
        TemplateName = "ABL ORD T86 MANIFEST-65CTNS";
        TemplateHeaderMatchingJson = "TemplatesBinding/USTemplates/HeaderMatchings/ABLORDT86.json";
    }
    public override XLWorkbook GenerateFile(List<int> rowsToExport, List<Dictionary<string, object>> rows)
    {
        ...
        foreach (int rowNumber in rowsToExport)
        {
            ...
            foreach (string header in rowToExport.Keys)
            {
                ...
                {
                    outputWorksheet.Cell(rowCount, 7).Value = ExtractSubstring(cellValue.ToString(), -1, 12);
                }
            }
